...
//Cleric
this.clericTable = [
{ level: 1, expPoints: 0, hitDice: '1d6', spells: ['-', '-', '-', '-', '-', '-'] },
{ level: 2, expPoints: 1500, hitDice: '2d6', spells: ['1', '-', '-', '-', '-', '-'] },
{ level: 3, expPoints: 3000, hitDice: '3d6', spells: ['2', '-', '-', '-', '-', '-'] },
{ level: 4, expPoints: 6000, hitDice: '4d6', spells: ['2', '1', '-', '-', '-', '-'] },
{ level: 5, expPoints: 12000, hitDice: '5d6', spells: ['2', '2', '-', '-', '-', '-'] },
{ level: 6, expPoints: 24000, hitDice: '6d6', spells: ['2', '2', '1', '-', '-', '-'] },
{ level: 7, expPoints: 48000, hitDice: '7d6', spells: ['3', '2', '2', '-', '-', '-'] },
{ level: 8, expPoints: 90000, hitDice: '8d6', spells: ['3', '2', '2', '1', '-', '-'] },
{ level: 9, expPoints: 180000, hitDice: '9d6', spells: ['3', '3', '2', '2', '-', '-'] },
{ level: 10, expPoints: 270000, hitDice: '9d6+1', spells: ['3', '3', '2', '2', '1', '-'] },
{ level: 11, expPoints: 360000, hitDice: '9d6+2', spells: ['4', '3', '3', '2', '2', '-'] },
...
//Character Sheet
    ...
    if(this.class === 'Magic User' || 'Cleric'){
...
getMagic(characterClass, characterLevel) {
  ...
  if (classTable) {
    const levelEntry = classTable.find(entry => entry.level === parseInt(characterLevel, 10));

    if (levelEntry) {
      console.log(characterClass, characterLevel, levelEntry);
      let levelCheck = 1

      // Loop through the spells array
for (let i = 0; i < characterLevel; i++) {
  const spellCount = levelEntry.spells[i];
  console.log(`${this.name} ${characterClass} Level ${i + 1}: ${spellCount} spells`);

  const spells = spells.spellsArray.filter(spell => spell.Level === (i + 1).toString() && spell.Class === characterClass);
  ...
  if (spells.length > 0) {
      for (let j = 0; j < spellCount; j++) {
          ...
          if(levelCheck < chosenSpell.level){
            this.magic += `<br> LEVEL ${i + 1} SPELLS <br>`;
            this.magic += `${chosenSpell} <br>`;
        ...
        // You need to replace this with the actual spell name for the given spell level
  ...
  return null; // Return null if classTable or levelEntry is not found
}
...
rollScore() {
  ...
  getHitPoints(){
    ...
    this.hitpoints = this.level * 8;
  
    }

    else if (this.class === 'Cleric'){

    this.hitpoints = this.level * 6;
